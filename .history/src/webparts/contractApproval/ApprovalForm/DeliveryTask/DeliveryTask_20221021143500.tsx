...
export class DeliveryTask extends React.Component<IDeliveryTaskProps, IDeliveryTaskState> {
            ...
            <div className={styles.viewdetail_block}>
              ...
              <div id="DeliveryTaskForm">
                ...
                <div id="LegalPendingModal" className={styles.modal}>
                  <div className={styles.modalcontent}>
                    <span className={styles.close} onClick={() => this.Close4()}>
                      &times;
                    </span>
                    <label className={styles.header2}>Legal feedback is pending!</label>
                  </div>
                </div>
  ...
  private async setForm() {
    ...
    this.webURL.lists.getByTitle("DeliveryFeedback").items.filter(filterStr).get().then(data => {
      ...
      this.webURL.lists.getByTitle("DeliveryFeedback").items.filter(filterStr4).getAll().then((items) => {
        if (items.length !== 0) {
          $("#DelDraft").hide();
        }
        else {
  ...
  private async loadQuestions() {
    this.webURL.lists.getByTitle("FeedbackQuestions").items.orderBy('QuestionNumber').filter("Team eq 'Delivery'").orderBy('Sequence').get().then((item) => {
  ...
  private async DelSubmitData() {
    ...
    this.webURL.lists.getByTitle("FeedbackQuestions").items.filter("Team eq 'Delivery'").get().then((DelData) => {
  ...
  private async DelSubmitData1() {
    const itemID = new URLSearchParams(window.location.search).get('itemid');
    this.webURL.lists.getByTitle("Projects").items.getById(parseInt(itemID)).select('Id', 'Status', 'DeliveryDone', 'LegalDone').get().then((Pdata) => {

      if (Pdata.DeliveryDone != 2) {
        if (Pdata.LegalDone !== -1 && Pdata.Status == "Delivery Action Awaited" && Pdata.Status !== "Management Action Awaited" && Pdata.Status !== "Escalated to management") {
        this.DelSubmitData2();
        }else {
          var modal = document.getElementById("LegalPendingModal");
  ...
  private async DelSubmitData2() {
    const itemID = new URLSearchParams(window.location.search).get('itemid');
    var filterStr = "ProjectID eq '" + itemID + "'";
    this.webURL.lists.getByTitle("DeliveryFeedback").items.filter(filterStr).get().then(async (feedbacks) => {
      if (feedbacks.length == 0) {
        this.webURL.lists.getByTitle("FeedbackQuestions").items.filter("Team eq 'Delivery'").get().then(async (DelData) => {
          var DelDatalength = DelData.length;
          for (var i = 0; i < DelDatalength; i++) {
            var ID = DelData[i].QuestionNumber;
          ...
          this.webURL.lists.getByTitle("Projects").items.getById(parseInt(itemID)).select('Id', 'Status', 'DeliveryDone', 'LegalDone').get().then(async (data) => {
            status = data.Status;
          ...
          this.webURL.lists.getByTitle("ProjectTasks").items.filter(filterStr5).getAll().then((items) => {
            if (items.length > 0) {
        ...
        this.webURL.lists.getByTitle("DeliveryFeedback").items.filter(filterStr).get().then(async (DeleteOld) => {
          for (var remove = 0; remove < DeleteOld.length; remove++) {
            this.webURL.lists.getByTitle('DeliveryFeedback').items.getById(DeleteOld[remove].ID).delete();
          }
        }).then(() => {
          ...
          this.webURL.lists.getByTitle("Projects").items.getById(parseInt(itemID)).select('Id', 'Status', 'DeliveryDone', 'LegalDone').get().then((data) => {
            status = data.Status;
            this.webURL.lists.getByTitle("Projects").items.getById(parseInt(itemID)).update({
              DeliveryDone: 1
            }).then(changestatus => {
    ...
    this.webURL.lists.getByTitle("DeliveryFeedback").items.filter(filterStr).get().then(async (feedbacks) => {
      if (feedbacks.length == 0) {
        ...
        this.webURL.lists.getByTitle("DeliveryFeedback").items.filter(filterStr).get().then(async (DeleteOld) => {
          ...
          this.webURL.lists.getByTitle("FeedbackQuestions").items.filter("Team eq 'Delivery'").get().then(async (DelData2) => {
            var Deliverydatalength = DelData2.length;
            for (var i = 0; i < Deliverydatalength; i++) {
              var ID = DelData2[i].QuestionNumber;
              var Title = $('#Question' + [ID])[0].innerHTML;
