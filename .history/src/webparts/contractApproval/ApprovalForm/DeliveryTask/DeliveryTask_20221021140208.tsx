...
export class DeliveryTask extends React.Component<IDeliveryTaskProps, IDeliveryTaskState> {
  ...
  private async loadQuestions() {
    this.web.lists.getByTitle("FeedbackQuestions").items.orderBy('QuestionNumber').filter("Team eq 'Delivery'").orderBy('Sequence').get().then((item) => {
  ...
  private async DelSubmitData() {
    ...
    this.web.lists.getByTitle("FeedbackQuestions").items.filter("Team eq 'Delivery'").get().then((DelData) => {
  ...
  private async DelSubmitData1() {
    const itemID = new URLSearchParams(window.location.search).get('itemid');
    this.web.lists.getByTitle("Projects").items.getById(parseInt(itemID)).get().then((Pdata) => {
