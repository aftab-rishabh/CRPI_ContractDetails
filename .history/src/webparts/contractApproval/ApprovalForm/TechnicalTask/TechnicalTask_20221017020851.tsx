...
import { Icon, IStackTokens, mergeStyleSets, Stack } from '@fluentui/react';
...
export class TechnicalTask extends React.Component<ITechnicalTaskProps, ITechnicalTaskState> {
  ...
  private async setForm() {
    ...
    let web = Web(this.props.webURL);
    await web.lists.getByTitle("TechnicalFeedback").items.filter(filterStr).get().then(data => {
      ...
      let web = Web(this.props.webURL);
      await web.lists.getByTitle("TechnicalFeedback").items.filter(filterStr4).getAll().then((items) => {
        if (items.length !== 0) {
          $("#btnDraft").hide();
        }
        else {
  ...
  private async TechSubmitData() {
    var isvalid = true;
    // =========== Questions Validations ====================
    let web = Web(this.props.webURL);
    await web.lists.getByTitle("FeedbackQuestions").items.filter("Team eq 'Technical'").get().then((Technicaldata) => {
  ...
  private async TechSubmitData2() {
    ...
    let web = Web(this.props.webURL);
    await web.lists.getByTitle("TechnicalFeedback").items.filter(filterStr).get().then(async (feedbacks) => {
      if (feedbacks.length == 0) {
        let web = Web(this.props.webURL);
        await web.lists.getByTitle("FeedbackQuestions").items.filter("Team eq 'Technical'").get().then(async (Technicaldata) => {
          ...
          for (var i = 0; i < Tdatalength; i++) {
            ...
            let web = Web(this.props.webURL);
            await web.lists.getByTitle("TechnicalFeedback").items.add({
              Title: TitleHTML,
              ProjectID: itemID,
              Department: Dpt,
        ...
        let web = Web(this.props.webURL);
        await web.lists.getByTitle("TechnicalFeedback").items.filter(filterStr).get().then(async (DeleteOld) => {
          for (var remove = 0; remove < DeleteOld.length; remove++) {
            let web = Web(this.props.webURL);
            await web.lists.getByTitle('TechnicalFeedback').items.getById(DeleteOld[remove].ID).delete();
          }
        }).then(async () => {
          let web = Web(this.props.webURL);
          await web.lists.getByTitle("FeedbackQuestions").items.filter("Team eq 'Technical'").get().then(async (Technicaldata2) => {
      ...
      let web = Web(this.props.webURL);
      await web.lists.getByTitle("TechnicalComments").items.filter(filterStr4).getAll().then((items) => {
        if (items.length > 0) {
          items.forEach(async (item4, index4) => {
            let web = Web(this.props.webURL);
            await web.lists.getByTitle("TechnicalComments").items.getById(item4.Id).update({
              IsFinished: true,
            }).then(async s => {
              var TechName = new URLSearchParams(window.location.search).get('dpt');
              var filterStr5 = "ProjectID eq '" + itemID + "' and " + "TaskType eq 'Technical'";
              let web = Web(this.props.webURL);
              await web.lists.getByTitle("ProjectTasks").items.filter(filterStr5).getAll().then(itemz => {
                if (itemz.length > 0) {
                ...
                let web = Web(this.props.webURL);
                await web.lists.getByTitle("TechnicalComments").items.filter(filterStr3).get().then(async data3 => {
                  if (data3.length == 0) {
                    var filterStr6 = "ProjectID eq '" + itemID + "' and " + "Title eq 'Delivery Action Awaited'";
                    let web = Web(this.props.webURL);
                    await web.lists.getByTitle("ProjectTasks").items.filter(filterStr6).getAll().then(async DelTask => {
