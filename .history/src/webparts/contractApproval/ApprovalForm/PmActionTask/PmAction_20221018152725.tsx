        ...
        web.getFolderByServerRelativeUrl(hostUrl).files.get().then(files => {
            if (files.length == 0) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    if (files[i].Title != "Management" && files[i].Title != "Estimation file" && files[i].Title != "Others" && files[i].Title != "Legal Documents File" && files[i].Title != "Project Related Document from CM" && files[i].Title != "Kick-off document" && files[i].Title !="Contract - Commercial") {
                        ...
                        attachmentfiles += `<li id="FileListItem` + i + `" value=` + valu + `><button class="CloseBtn"`
                            ...
                            + `${files[i].Name}</a></li>`;
                            (document.getElementById('FileList2') as HTMLInputElement).innerHTML = attachmentfiles;
