...
export default class ContractDetailsNew extends React.Component<IContractDetailsNewProps, IContractDetailsNewState> {
    ...
    return (
      <div className={styles.contractDetailsNew}>
        ...
        <div className={styles.viewdetail_block}>
          ...
          style={{ marginRight: '10px' }}> </PrimaryButton>
          <PrimaryButton text="Chat" onClick={() => this.ChatBox()}> </PrimaryButton>
            </div></div>
  ...
  private async setData() {
    ...
    .items.getById(parseInt(itemID)).select('*', 'Author/Id','Author/Email','DeliveryManager/Title', 'AccountManager/Title').expand('DeliveryManager/Id', 'AccountManager/Id','Author').get().then(async (item) => {         
