...
export default class ViewContracts extends React.Component<IViewContractsProps,IViewContractsState,{}> {
 ...
 this._columns = [
    ...
    { 
     ...
     minWidth: 185, 
      ...
      isPadded: true,
      isResizable: true,
      onRender: (data) => (this._stausobject[data.Stage] == null ? data.Stage : this._stausobject[data.Stage]) 
    },  
    { 
      key: "EstimatedHours", 
      name: "Est. Hours",
    ...
    { 
      ...
      onRender: (data) =>
        ...
        (
        <>
       <Link href={this._ContextUrl + "/SitePages/contract-details-New.aspx?itemid=" + data.ID} target="_blank" data-interception="off" style={{
           padding:'5px',
      ...
      (data.CreatedBY.toLowerCase() == this.props.userEmailId.toLowerCase() || this.props.userEmailId.toLowerCase() == 'Sapan.Sehgal@rishabhsoft.com'.toLowerCase())
       
      ? 
      
    ( 
      
   (data.Stage == "Technical Action Awaited" || data.Stage == "Legal Action Awaited" 
      || data.Stage == "Delivery Action Awaited" 
     || data.Stage == "Management Action Awaited" ) 
      ?
      
      <><a href={this._ContextUrl + "/SitePages/UpdateContract.aspx?itemid=" + data.ID} target="__blank"> 
         <IconButton
      ...
      onClick={() => this.setState({ hideDialog: false,itemID:data.ID })}
      styles={{
       icon: { color: "#4672c4", fontSize: 18 },
       root: {
         selectors: {
...
public componentDidMount() {
  this._spServices.ProjectsListDataInProgress().then((result) => {
    setTimeout(() => {
      this.setState({ListData:result});   
    },1000);
  });
  this._spServices.ProjectsListData().then((result) => {
    setTimeout(() => {
...
const sbThumbBg = "red";
    ...
    <div className={styles.viewContracts}>
     ...
     onClick={() => {this._spServices.ProjectsListDataInProgress().then((result) => {
      setTimeout(() => {
        this.setState({ListData:result,CurrentTab:"In Progress"});
      }, 1000);
     })}}>In-Progress</PrimaryButton>
     ...
     onClick={() => {this._spServices.ProjectsListDataCompleated().then((result) => {
      setTimeout(() => {
     ...
     <PrimaryButton
      className={
       ...
       onClick={() => {this._spServices.ProjectsListData().then((result) => {
        setTimeout(() => {
...
<DetailsList
    ...
    items={this.state.CurrentTab == "All Data" ? this.state.ListAllData : this.state.ListData }
    compact={true}
...
<div id="loader" className={styles.modal}>
  ...
  private async DeleteContract() {
    ...
    await web.lists
    .getByTitle("Projects").items.getById(parseInt(_itemID)).update({
      Status: "Deleted",
    })
    .then(async (i) => {
        console.log("loading...");
