...
export class Accordion extends React.Component<IAccordionProps, IAccordionState> {
  ...
  public async fetchData() {
    ...
    console.log(items);
    var hostUrl =  '/sites/demo/ProjectDocuments/' + this.state.OppID;
    const ContractFiles: any[] = await web.getFolderByServerRelativeUrl(hostUrl).files.get();
    this.setState({ Items: items, files:ContractFiles });
...
<div className={styles.sectionblock}>
            ...
            <div className={styles.view_listing}>
             ...
             <li>{title}<a href={item.ServerRelativeUrl} target="_blank">{item.Name}</a></li>
