...
// export class LegalTask extends React.Component<
//   ILegalTaskProps,
//   ILegalTaskState
// > {
//   ContractStatus = "";
...
//   private Close0() {
//     let modal = document.getElementById("ExistingModal");
...
//         fileArr.push(file);
//         let hostUrl =
//           this.relativeUrl + "/ProjectDocuments/" + $("#oppID").text();
//         this.webURL
...
//     // let hostUrl = this.context.pageContext.site.serverRelativeUrl+ '/ProjectDocuments/'+$('#oppID').text();
//     let hostUrl = this.relativeUrl + "/ProjectDocuments/" + $("#oppID").text();
//     this.webURL
//       .getFolderByServerRelativeUrl(hostUrl)
//       .files.filter("Title eq 'Legal Documents File'")
//       .get()
//       .then((files) => {
//         if (files.length !== 0) {
...
//               `id="CloseBtn" style="padding: 0;">&times;</button><Label style="display:none;">` +
//               files[i].UniqueId +
//               `</Label>` +
//               `<Label id="filesnames` +
//               i +
...
//       .getByTitle("Projects")
//       .items.getById(parseInt(itemID))
//       .select("Id", "Status", "DeliveryDone", "TechnicalDone", "LegalDone")
//       .get()
//       .then((data) => {
...
> {
  ...
  private setForm() {
    const itemID = new URLSearchParams(window.location.search).get("itemid");
    this.webURL.lists.getByTitle("Projects").items.getById(parseInt(itemID)).get().then((data) => {
        if (data.FeedbackStatus == "Submit") {
          $("#LegalDraft").hide();
        } else {
          $("#LegalDraft").show();
        ...
        this.setState({
          LegalCommentVal: data.LegalComments == null ? "" : data.LegalComments
        });
  ...
  private onRichTextChange(newValue: string): string {
    ...
    this.setState({ LegalCommentVal: newValue });
    return newValue;
  }
                ...
                <div className="row">
                  <div className="col col-lg-12">
                    <div className={styles.form_group}>
                      ...
                      <div id="LegalCommentDiv" className={styles.richTextContainer} >
                      <RichText 
                      className="LegalCommentElement"
                      value={this.state.LegalCommentVal} 
  ...
  private LegSubmitData() {
    ...
    if (this.state.LegalCommentVal.replace(/<\/?[^>]+(>|$)/g, "").replace(/&nbsp;/g, "").trim().length === 0) {
      $("#LegalErr").show();
      isvalid = false;
    } else $("#LegalErr").hide();
    if (isvalid) {
    ...
    this.webURL.lists
      ...
      .then((data) => {
        ...
        this.webURL.lists
          ...
          .items.getById(parseInt(itemID))
          .update({
            LegalComments: this.state.LegalCommentVal,
            Status: status,
            LegalDone: 1,
            FeedbackStatus: "Submit",
          })
